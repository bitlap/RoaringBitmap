...
public class RunArrayOrBenchmark {
  ...
  public int OrBitmapContainer(BenchmarkState benchmarkState) {
    ...
    for (int k = 1; k < benchmarkState.ac.size(); ++k)
      answer +=
          RoaringBitmap.or(benchmarkState.ac.get(k - 1), benchmarkState.ac.get(k)).getCardinality();
    return answer;
  }

  @Benchmark
  public int AndConcise(BenchmarkState benchmarkState) {
    int answer = 0;
    for (int k = 1; k < benchmarkState.cc.size(); ++k)
      answer += benchmarkState.cc.get(k - 1).intersection(benchmarkState.cc.get(k)).size();
    return answer;
  }
